...
export default function CategoriesScreen({ navigation, route }) {
	...
	const renderItem = ({ item }) => {
		...
		>
			{
				item.imagem && <Avatar size="medium" source={{uri: item.imagem.url}} />
			}
			<ListItem.Content>
				<ListItem.Title>
					{item.nome}
				</ListItem.Title>
				{item.publicado_em && <ListItem.Subtitle>{moment(item.publicado_em).format('YYYY')} - {item.artistas[0] && item.artistas[0].nome}</ListItem.Subtitle>}
			</ListItem.Content>
			<ListItem.Chevron />
		</ListItem>)};
	...
	if (refreshing)
		return (
			<View style={styles.notfound}>
				<Card.Title style={styles.notfoundTitle}>Carregando os Albums...</Card.Title>
				<Card.Divider />
			</View>
		);
